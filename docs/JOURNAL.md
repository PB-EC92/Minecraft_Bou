...
**Tests.** 547 tests unitaires (527 au J6 ; nouveaux : enfant coincé, enclos, import tout ou rien et restauration sans effacement, copies de secours exportées, Pixel sur la terre, caméra en diagonale, compte à l'objectif, conseil sac plein, identifiants de blocs figés, réglage `devTools`). Tests de fumée : 102 exécutions sur les trois profils, toutes vertes (96 au J6) ; nouveaux cas : écran des enfants sans panneau Tests (et le mode parent qui le réaffiche), coincé au fond d'un puits (au clavier et au doigt), sac plein pendant l'étape « 6 troncs », aucune requête réseau au démarrage, icône de la pierre brillante, compte dit à l'objectif, pas de « grimpe » dans son propre abri, message « ouvert ailleurs » sur l'accueil. Les cas de mission ont des délais d'attente plus longs : chaque « bravo » attend désormais la fin de sa phrase avant l'étape suivante. Sous forte charge (6 processus), les cas de mission passent 21 fois sur 21 ; un cas du tutoriel qui dépendait de la capture de la souris sous charge pose maintenant son bloc par l'accès de test (le clic droit reste vérifié par le cas du J2). `dist/cubes.html` : 706 ko ; version « 1.0-rc ».

**Non vérifié.** Tout sur Firefox et le convertible ; la voix réelle (durée des phrases, coupures) ; les critères de réussite du brief avec les enfants.

**Attendu de Pierre.** Exporter les sauvegardes, remplacer le fichier au même endroit ; dérouler le protocole « Recette V1 » de `RETOURS.md` (d'abord la ligne « stockage sous Firefox » du protocole J4, puis la séance avec les deux enfants) ; trancher les cinq décisions ouvertes de `RECETTE-V1.md` §5 (vider une case, clôture avec chaque tronc, nuit avant l'abri, indication du mode parent, découverte de la lampe suggérée). Après la séance et ses corrections : version « 1.0 ».

**Prochaine étape.** Corrections d'après la recette avec les enfants, puis « 1.0 » (`src/game/identity.ts`, `package.json`).
