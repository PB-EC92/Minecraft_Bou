...
Jeu de construction en blocs 3D, éducatif, pour deux enfants. Fonctionne dans un navigateur, hors ligne, sans rien installer. État : **version 1.0-rc** (candidate à la V1, en recette avec les enfants) : monde généré (quatre types), jour et nuit, eau ; casse par appui maintenu, sac de 9 cases, compte lu à voix haute ; commandes tactiles pour le convertible replié ; accueil avec deux profils, trois mondes enregistrés par enfant, mode parent, personnage visible à la troisième personne ; compagnon Pixel, Grignotes la nuit, lampe, clôture et lance-bulles ; **tutoriel, mission 1 « Construis un abri avant la nuit » jouable de bout en bout, écran de félicitations et cadeau** (voir `docs/PLAN.md` et `docs/JOURNAL.md`).

## Jouer

Ouvrir `dist/cubes.html` par double-clic (Chrome, Edge ou Firefox). C'est tout : le fichier contient le jeu entier. La ligne d'infos en haut à gauche se termine par la version (« 1.0-rc ») ; elle est visible une fois l'option « Afficher le panneau Tests et les infos techniques » cochée dans le mode parent (masquée par défaut pendant les parties des enfants) : si ce n'est pas le cas, c'est une ancienne copie.

Sur PC : cliquer sur le monde pour capturer la souris. **ZQSD** (ou flèches) pour bouger, **Espace** pour sauter (ou nager vers le haut), **Maj** pour plonger, **clic gauche maintenu** pour casser (un anneau se remplit autour du viseur ; une fleur se cueille avec un appui plus court, mais un clic bref ne suffit pas), **clic droit** pour poser le bloc de la case choisie, **1 à 9** ou la **molette** pour choisir une case, **Échap** pour libérer la souris. Les marches d'un bloc se montent sans sauter ; dans l'eau, on flotte. Tombé dans un trou ? Garder **Espace** en avançant contre la paroi : au bout d'un instant, on grimpe (Pixel le rappelle si l'enfant pousse en vain contre les parois) (un mur de deux blocs arrête toujours si l'on marche sans sauter). En gardant le clic, on casse les blocs à la suite, mais un seul plus bas que ses pieds par appui. Si le navigateur refuse la capture, le jeu passe en mode repli : glisser en tenant le bouton pour regarder, bouton gauche maintenu sans bouger pour casser (on peut aussi viser en glissant puis s'arrêter sans lâcher), clic droit sans bouger pour poser.
...
**Mode parent** : sur l'écran « Qui joue ? », garder le doigt (ou le clic) **3 secondes** sur l'engrenage en bas à gauche. On y change les prénoms, niveaux et voix, la durée de la nuit (normale, courte, pas de nuit), les Grignotes, l'affichage du panneau « Tests » et des infos techniques pendant les parties (décoché par défaut : les enfants ne les voient pas), on suit la progression de chacun, on efface un monde, on **exporte** tout dans un fichier (`cubes-sauvegarde-AAAA-MM-JJ.json`, dans les téléchargements) et on l'**importe** (sur ce convertible ou un autre appareil). Exporter avant chaque nouvelle version du jeu. Ouvrir le jeu avec `#monde=…&graine=…` dans l'adresse saute l'accueil et n'enregistre rien (tests) ; `&mission=0` y ajoute Pixel et le tutoriel, `&mission=1` la mission 1, `&creatures=1` les Grignotes.

### Pixel, les Grignotes et la nuit (J5)

**Pixel**, le renard, trottine devant l'enfant et lit la mission en cours dans le bandeau en haut de l'écran (bouton **haut-parleur** pour la réentendre) ; l'avancement s'affiche (« 3 / 6 »). Casser une **pierre brillante** (cristaux bleus, en surface) donne une **lampe** ; casser un tronc donne aussi une **clôture**.
...
### Tutoriel, mission 1 et félicitations (J6)

Dans un nouveau monde, Pixel commence par le **tutoriel** : marcher (le rond au doigt, **Z** au clavier), casser un bloc, le poser. Il n'est fait qu'une fois par enfant : ses mondes suivants commencent directement à la mission 1 (le mode parent indique « tutoriel fait »).

**Mission 1, « Construis un abri avant la nuit »** : ramasser 6 troncs, puis 4 pierres ; construire un **abri** et entrer dedans ; trouver une pierre brillante (elle donne une lampe) ; poser la lampe près de l'abri ; rester près de la lampe : le temps file jusqu'à la nuit, et l'on voit les Grignotes s'enfuir devant la lumière. Un abri, pour le jeu : un bloc plein au-dessus de la tête (jusqu'à 4 blocs plus haut), des murs dans au moins trois directions (à 4 blocs au plus, à hauteur des pieds ou de la tête : la porte peut rester ouverte), et au moins un bloc posé par l'enfant (un terrier creusé dans une colline compte aussi ; un sous-bois naturel, non). Pendant cette étape, un petit dessin de maison dans le bandeau s'allume partie par partie (toit, murs) quand l'enfant est dedans, et Pixel dit ce qui manque. Astuce pour le toit : avec des murs de trois blocs de haut, on le pose depuis l'intérieur en visant le haut du mur.

À la fin, un **écran de félicitations** (prénom, ce qui a été réuni, confettis) et un **cadeau** : 5 blocs arc-en-ciel. Si le sac est plein, le cadeau attend qu'une case se libère. Le monde réussi porte une **étoile** à l'accueil ; le mode parent affiche la progression de chaque monde. Rien ne bloque : sans nuit (réglage « pas de nuit ») ou sans Grignotes, la dernière étape se valide seule.

### Le sac

Chaque bloc cassé est ramassé : il rejoint la case de son type dans la barre du bas (9 cases, 99 blocs au plus par case), et son nombre s'affiche. Poser un bloc en retire un. Le jeu annonce le compte (« 3 pierres ! ») à l'écran ; pour le lecteur débutant, la voix le lit au premier bloc de chaque sorte puis tous les 5 blocs (5, 10, 15…), pour ne pas se répéter. Les planches ne se trouvent pas dans la nature pour l'instant (le craft est prévu en V2) : le bouton **Compléter le sac** du panneau « Tests » (réservé à l'adulte) complète jusqu'à 20 blocs de chaque sorte, dans la limite des 9 cases (ce qui a été ramassé reste ; « Vider le sac » d'abord pour avoir tout le kit). Le panneau règle aussi le niveau de lecture (débutant : messages courts lus à voix haute ; autonome : phrases complètes, voix coupée), la voix et les sons.

### Pour l'adulte : panneau « Tests » et adresse

Les enfants choisissent leur monde à l'accueil (J4). Le panneau **Tests** (en haut à droite, visible en mode adresse, ou dans les parties des enfants si le mode parent l'autorise) sert aux vérifications : type de monde et graine (même type + même graine = le même monde ; « Nouveau monde » est refusé pendant la partie d'un enfant pour ne pas écraser son monde), heure (matin, midi, soir, nuit, temps accéléré), distance de rendu (96 blocs par défaut, choix retenu d'une ouverture à l'autre), niveau de lecture, voix, sons, « Compléter le sac » et diagnostic. Ouvrir le jeu avec une adresse du type `cubes.html#monde=ile&graine=1234` saute l'accueil et n'enregistre rien ; on peut y ajouter `&heure=20`, `&distance=48`, `&mission=0` ou `&mission=1`, `&creatures=1`.
